...
| **Phase 3.93: 教材名稱變更與 AI 全新生成多例句 JSON 重構** | 修改教材名稱為「高中單字 L3」；採著作權合規方案：例句與翻譯全面由 AI 重新生成。在此階段整合：(1) Google 帳號連結成功後自動刷新單元選單；(2) 玩家未滿 5 字時手動測驗結束清零計數器；(3) 列表模式搜尋框縮短防止 iPhone XS 上跑版；(4) 字卡模式下控制按鈕依「篩選」條件動態顯示；(5) 測驗模式下題目例句一律顯示英文，且生命值剩一命時自動顯示該例句的中文翻譯。 | 已完成 |
